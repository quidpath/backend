...
class BillingDashboardAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['billing_stats'] = stats_result.get('data', {}) if stats_result.get('success') else {}
        
        # Get recent trials
        trials_result = self.billing_client.admin_list_trials(limit=10)
        extra_context['recent_trials'] = trials_result.get('data', {}).get('trials', []) if trials_result.get('success') else []
        
        # Get active subscriptions
        subs_result = self.billing_client.admin_list_subscriptions(status='active', limit=10)
        extra_context['active_subscriptions'] = subs_result.get('data', {}).get('subscriptions', []) if subs_result.get('success') else []
        
        # Get pending invoices
        invoices_result = self.billing_client.admin_list_invoices(status='pending', limit=10)
        extra_context['pending_invoices'] = invoices_result.get('data', {}).get('invoices', []) if invoices_result.get('success') else []
...
def billing_trials_view(request):
    ...
    if result.get('success'):
        ...
        return render(request, 'admin/billing_error.html', {'error': result.get('message')})
...
def billing_subscriptions_view(request):
    ...
    if result.get('success'):
        ...
        return render(request, 'admin/billing_subscriptions.html', {'subscriptions': subscriptions})
    else:
        return render(request, 'admin/billing_error.html', {'error': result.get('message')})
