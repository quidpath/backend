...
class SESAdapter(MessagingAdapter):
    ...
    def send(
        ...
        if not subject:
            subject = (
                metadata.get("subject", "Notification") if metadata else "Notification"
            )
        ...
        message_body = {"Html": {"Charset": "UTF-8", "Data": message}}
        ...
        if metadata and "reply_to" in metadata:
            email_params["ReplyToAddresses"] = (
                metadata["reply_to"]
                if isinstance(metadata["reply_to"], list)
                else [metadata["reply_to"]]
            )
