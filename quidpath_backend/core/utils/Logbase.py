...
from Authentication.models.logbase import Transaction, State, TransactionType
...
class TransactionLogBase:
    ...
    @classmethod
    def log(cls, transaction_type: str, user=None, message="", state_name="Active", extra=None, notification_resp=None, request=None):
        """
    ...
    def _log_transaction(self, transaction_type, user=None, message="", state_name="Active", extra=None, notification_resp=None, request=None):
        """
        ...
        try:
            with transaction.atomic():
                ...
                txn_data = {
                    ...
                    "amount": 0,
                    "message": message,
                    "response": "200.000.000" if state_name == "Completed" else "400.000.000",
                    "source_ip": source_ip or "0.0.0.0",
                    "state": state,
                    "notification_response": str(notification_resp) if notification_resp else None,
                }

                transaction_obj = Transaction.objects.create(**txn_data)

                logger.info(f"[TransactionLog] ✅ {transaction_type} | user={user_instance} | state={state_name}")
    ...
    def _get_transaction_type(self, txn_name: str) -> TransactionType:
        ...
        return TransactionType.objects.get_or_create(
            name=txn_name,
            defaults={"simple_name": txn_name, "class_name": txn_name}
        )[0]
