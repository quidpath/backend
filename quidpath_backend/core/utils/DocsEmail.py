...
class DocumentNotificationHandler(TemplateManagementEngine):
    ...
    def send_document_notification(
        ...
        try:
            for data in notifications:
                ...
                log.info(f"Attempting to send document email to {destination}")
                ...
                if notification_response.get("code") == "200.001.001":
                    ...
                    log.info(f"Successfully sent document email to {destination}")
                else:
                    # Mark as failed
                    NotificationService().mark_failed(notification_obj.id)
                    notification_obj.response_payload = notification_response
                    notification_obj.save()
                    log.error(f"Failed to send document email to {destination}: {notification_response.get('message')}")
    ...
    def _send_email(
        ...
        log.info(f"Preparing to send document email to {recipient_email} with subject: {subject}")
        ...
        try:
            ...
            log.info(f"Document email sent successfully to {recipient_email}")
            return {
                "status": "success",
                "code": "200.001.001",
