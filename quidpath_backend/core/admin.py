...
class BillingOverviewAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('overview/', self.admin_site.admin_view(self.overview_view), name='billing_overview'),
            path('trials/', self.admin_site.admin_view(self.trials_view), name='billing_trials'),
            path('subscriptions/', self.admin_site.admin_view(self.subscriptions_view), name='billing_subscriptions'),
            path('invoices/', self.admin_site.admin_view(self.invoices_view), name='billing_invoices'),
            path('payments/', self.admin_site.admin_view(self.payments_view), name='billing_payments'),
        ]
    ...
    def overview_view(self, request):
        ...
        context = {
            ...
            'error': None if stats_result.get('success') else stats_result.get('message'),
        }
    ...
    def trials_view(self, request):
        ...
        context = {
            ...
            'opts': self.model._meta,
            'has_permission': True,
            'trials': result.get('data', {}).get('trials', []) if result.get('success') else [],
    ...
    def subscriptions_view(self, request):
        ...
        context = {
            ...
            'opts': self.model._meta,
            'has_permission': True,
            'subscriptions': result.get('data', {}).get('subscriptions', []) if result.get('success') else [],
            'error': None if result.get('success') else result.get('message'),
        }
    ...
    def invoices_view(self, request):
        ...
        context = {
            ...
            'opts': self.model._meta,
            'has_permission': True,
            'invoices': result.get('data', {}).get('invoices', []) if result.get('success') else [],
            'error': None if result.get('success') else result.get('message'),
        }
    ...
    def payments_view(self, request):
        ...
        context = {
            ...
            'opts': self.model._meta,
            'has_permission': True,
            'payments': result.get('data', {}).get('payments', []) if result.get('success') else [],
            'error': None if result.get('success') else result.get('message'),
        }
