...
class SubscriptionMiddleware:
    ...
    def __call__(self, request):
        ...
        try:
            ...
            if not access_result.get("success"):
                logger.error(
                    f"Billing service error for corporate {corporate_id}: {access_result.get('message')}"
                )
            ...
            if access_result.get("unpaid_invoices_count", 0) > 0:
                response["X-Quidpath-Unpaid-Invoices"] = str(
                    access_result["unpaid_invoices_count"]
                )
            ...
            logger.error(
                f"Error checking subscription for corporate {corporate_id}: {str(e)}"
            )
    ...
    def _extract_corporate_id(self, request) -> str:
        ...
        if hasattr(request, "user") and request.user.is_authenticated:
            ...
            if hasattr(request.user, "corporate") and hasattr(
                request.user.corporate, "id"
            ):
