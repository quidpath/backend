...
class StatementParser:
    ...
    def __init__(self, spec: StatementSpec, label_threshold: float = 0.75) -> None:
        ...
        self.matcher = AdvancedLabelMatcher(
            aliases=spec_aliases, fuzzy_threshold=85.0, partial_threshold=90.0
        )
        ...
        for sheet_name, df in tables.items():
                ...
                for row_idx, row_series in iterator:
                    ...
                    if first_label:
                        ...
                        if any(
                            ...
                            if not any(
                                term in first_label_lower
                                for term in ["total", "expense", "cost"]
                            ):
                    ...
                    for col_idx in range(len(row_list)):
                        ...
                        if (
                            ...
                            logger.debug(
                                "Re-mapping 'Total Operating Income' from operating_income → total_revenue"
                            )
                        ...
                        is_section_header = (
                            (
                                ...
                                or "sales" in label_lower
                                or "expense" in label_lower
                        ...
                        if is_section_header:
                            ...
                            logger.debug(
                                "Skipping section header '%s' - no numeric values",
                                label_text,
                            )
                            continue
                        ...
                        if field in self.spec.optional_fields and field == "risk_level":
                            value = self._find_value_in_row(
                                row_list, col_idx, is_numeric=False
                            )
                            if value:
                        ...
                        value = self._find_value_in_row(
                            row_list, col_idx, is_numeric=True
                        )
                        print(
                            f"   Extracted value: {value} (type: {type(value).__name__})"
                        )
                        if value is not None:
                            ...
                            for idx, cell in enumerate(row_list):
                                print(
                                    f"  [{idx}]: {repr(cell)} (type: {type(cell).__name__})"
                                )
                                logger.info(
                                    "  [%d]: %s (type: %s)",
                                    idx,
                            ...
                            fallback_value = extract_numeric_value(
                                row_list, label=label_text, search_entire_row=True
                            )
            ...
            for field, candidates in candidate_matches.items():
                ...
                if field in summable_fields and len(candidates) > 1:
                    ...
                    if valid_total:
                        ...
                        candidates = [valid_total] + [
                            c for c in candidates if c != valid_total
                        ]
                    else:
                        ...
                        non_zero_items = [
                            c
                            for c in candidates
                            if c[1] != 0 and "total" not in c[0].lower()
                        ]
                ...
                if len(candidates) > 1 and not should_sum:
                    logger.debug(
                        "   Skipped %d other candidates for %s",
                        len(candidates) - 1,
                        field,
                    )
            ...
            if sheet_extracted.get("other_income"):
                ...
                if other_inc > 0 and potential_sales > 0:
                    ...
                    other_percentage = (
                        (other_inc / total_rev * 100) if total_rev > 0 else 0
                    )
                    ...
                    if other_percentage < 5:
                        ...
                        logger.info(
                            "Added other_income to total_revenue: %s",
                            expected_with_other,
                        )
                else:
                    print(
            ...
            if (
                ...
                and sheet_extracted.get("interest_expense") is not None
            ):
            ...
            for required_field, optional_field in fallback_mappings.items():
                if (
                    sheet_extracted.get(required_field) is None
                    and sheet_extracted.get(optional_field) is not None
                ):
            ...
            if self._has_all_required(sheet_extracted):
                ...
                validated_payload = {
                    k: v
                    for k, v in sheet_extracted.items()
    ...
    def _find_value_in_row(
        self, row_list: List[Any], label_col_idx: int, is_numeric: bool
    ) -> Optional[Any]:
        """
    ...
    def _coerce_label(self, value: Any) -> Optional[str]:
        ...
        if numeric_chars and len(numeric_chars) > 0:
            digit_ratio = (
                sum(1 for c in numeric_chars if c.isdigit()) / len(numeric_chars)
                if numeric_chars
                else 0
            )
            if (
                digit_ratio > 0.8 and len(numeric_chars) >= 3
    ...
    def _is_summary_row(self, label_text: str, row_list: List[Any]) -> bool:
        ...
        non_empty_cells = sum(
            1
            for cell in row_list
            if cell and str(cell).strip() and str(cell).strip().lower() != "nan"
        )
        if (
            non_empty_cells <= 3
        ):  # Summary rows typically have label + value + maybe one other field
            # Check if this looks like a total row
            if any(
                word in label_lower for word in ["total", "profit", "income", "loss"]
            ):
                return True
...
class IntelligentStatementExtractor:
    ...
    def __init__(
        self, specs: Tuple[StatementSpec, ...] = DEFAULT_STATEMENT_SPECS
    ) -> None:
        self.specs = specs
    ...
    def extract(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        for spec in self.specs:
            ...
            try:
                ...
                logger.error(
                    "Unexpected error parsing %s: %s", spec.name, exc, exc_info=True
                )
                continue
        ...
        if not statements:
            ...
            for spec in self.specs:
                logger.error(
                    f"   - {spec.name} (required fields: {spec.required_fields})"
                )
            logger.error(f"Available sheets: {list(tables.keys())}")
            logger.error("=" * 80)
            return {"success": False, "error": "No recognizable statements found."}

        primary_json = statements.get("income_statement") or next(
            iter(statements.values())
        )
        records = [
            {"statement_type": name, **payload} for name, payload in statements.items()
        ]
