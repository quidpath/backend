...
from .accounting_aliases import (ACCOUNTING_ALIASES, FIELD_PRIORITY,
    ...
    def match(self, label: str) -> Optional[str]:
        ...
        for field, alias_list in self.aliases.items():
            for alias in alias_list:
                ...
                if significant_alias_words and significant_alias_words.issubset(
                    label_words
                ):
                    # Calculate confidence based on word overlap
                    overlap_ratio = len(significant_alias_words) / max(
                        len(alias_words), 1
                    )
            ...
            for alias in alias_list:
                ...
                if RAPIDFUZZ_AVAILABLE:
                    ...
                    match_type = (
                        ...
                        else "partial" if partial_score == score else "token_set"
                    )
                else:
                    # Fallback to difflib
                    from difflib import SequenceMatcher
                ...
                if score > best_score:
                    ...
                    best_priority = (
                        FIELD_PRIORITY.get(best_match[0], 0) if best_match else 0
                    )
...
def match_label_to_field(
    label: str, aliases: Dict[str, List[str]] = None
) -> Optional[str]:
    """
