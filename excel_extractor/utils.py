...
def clean_numeric(value: Any) -> int:
    ...
    if isinstance(value, str):
        text = value.strip()
        if not text:
            raise ValueNormalizationError("Empty string cannot be normalized to a number.")
...
class LabelMatcher:
    ...
    def __init__(self, labels: Mapping[str, Sequence[str]], threshold: float = 0.75) -> None:
    ...
    def match(self, label: str) -> Optional[str]:
        ...
        for field, variations in self.labels.items():
            for variation in variations:
                ...
                if variation_words and label_words:
                    ...
                    if variation_words.issubset(label_words) or label_words.issubset(variation_words):
                        # Calculate score based on word overlap
                        overlap = len(variation_words & label_words) / max(len(variation_words), len(label_words))
                ...
                if any(word in normalized for word in variation.split() if len(word) > 3):
        ...
        if best_match and best_match.score >= self.threshold:
            logger.debug("✅ Matched label '%s' to field '%s' with score %.2f", label, best_match.field, best_match.score)
