...
class CorporateAdmin(admin.ModelAdmin):
    list_display = ("id","name", "email", "is_active", "billing_status", "is_approved", "created_at")
    ...
    fieldsets = (
        ('Company Information', {
            'fields': ('name', 'industry', 'company_size', 'description', 'website', 'logo')
        }),
        ('Contact Information', {
            'fields': ('email', 'phone', 'address', 'city', 'state', 'country', 'zip_code')
        }),
        ('Registration Details', {
            'fields': ('registration_number', 'tax_id', 'message')
        }),
        ('Status', {
            'fields': ('is_active', 'is_approved', 'is_rejected', 'rejection_reason', 'is_seen', 'is_verified')
        }),
        ('Billing Information', {
            'fields': ('get_billing_summary',),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
    ...
    def billing_status(self, obj):
        ...
        if has_access:
            if access_type == 'trial':
                ...
                return format_html('<span style="color: green;" title="Active">✅ Active</span>')
        else:
            return format_html('<span style="color: red;" title="Expired">❌ Expired</span>')
