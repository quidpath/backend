...
class FlutterwaveAdapter(PaymentAdapter):
    ...
    def handle_stk_callback(self, payload: Dict[str, Any], headers: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if event == 'charge.completed':
            ...
            if status == 'successful':
                return {
                    ...
                    'mpesa_receipt_number': data.get('processor_response', {}).get('ReceiptNumber', ''),
                ...
                return {
                    ...
                    'merchant_request_id': data.get('tx_ref', ''),
                    'result_code': '1',
                    'result_desc': data.get('processor_response', {}).get('ResponseDescription', 'Payment failed'),
                    'full_payload': payload
                }
        else:
            return {
    ...
    def initiate_card_payment(
        ...
        try:
            ...
            # Generate unique transaction reference
            import uuid
            tx_ref = metadata.get('transaction_reference', f"QUIDPATH-{uuid.uuid4().hex[:12]}")
            ...
            if data.get('status') == 'success':
                ...
                logger.error(f"Flutterwave card payment initiation failed: {error_message}")
                return {
                    'status': 'failed',
                    'redirect_url': None,
                    'transaction_id': None,
    ...
    def handle_card_webhook(self, payload: Dict[str, Any], headers: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if not self.verify_webhook_signature(json.dumps(payload, sort_keys=True).encode('utf-8'), headers):
    ...
    def verify_webhook_signature(self, payload: bytes, headers: Dict[str, Any]) -> bool:
        ...
        if not self.encryption_key:
            logger.warning("No encryption key configured for Flutterwave webhook verification")
        ...
        if hmac.compare_digest(expected_signature, signature):
            ...
            logger.error(f"Flutterwave webhook signature mismatch. Expected: {expected_signature}, Got: {signature}")
            return False
    ...
    def initiate_payment(
        ...
        if payment_method == 'mpesa':
            return self.initiate_stk_push(
                ...
                amount=float(amount),
                currency=currency,
                account_reference=metadata.get('invoice_id', 'QUIDPATH') if metadata else 'QUIDPATH',
                transaction_desc=metadata.get('description', 'Payment') if metadata else 'Payment',
