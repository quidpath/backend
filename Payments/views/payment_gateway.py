...
def initiate_mpesa_stk(request):
    ...
    try:
        ...
        metadata_dict = {
            ...
            "account_reference": (
                invoice.get("number", "QUIDPATH") if invoice else "QUIDPATH"
            ),
            "description": (
        ...
        with transaction.atomic():
            ...
            payment_data = {
                ...
                or result.get("merchant_request_id")
            ...
            return ResponseProvider(
                data={
                    ...
                    "message": result.get("customer_message")
                    or result.get("response_description")
                    or "STK Push initiated successfully",
                },
                message="STK Push initiated successfully",
                code=200,
            ).success()
        ...
        return ResponseProvider(
            message="An error occurred while initiating STK Push", code=500
        ).exception()
...
def mpesa_webhook(request):
    ...
    try:
        ...
        if not payments:
            logger.warning(
                f"Flutterwave M-Pesa webhook: Payment not found for {checkout_request_id}"
            )
        ...
        if not adapter.verify_webhook_signature(request.body, dict(request.headers)):
            logger.warning(
                f"Flutterwave M-Pesa webhook: Invalid signature for {checkout_request_id}"
            )
            return ResponseProvider(
                    ...
                    if invoices:
                        ...
                        registry.database(
                            ...
                            data={
                                ...
                                or webhook_data.get("checkout_request_id"),
                            },
                        )
