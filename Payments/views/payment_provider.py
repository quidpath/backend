...
def get_payment_provider(request, provider_type="flutterwave"):
    ...
    try:
        ...
        return ResponseProvider(
            message=f"Error getting payment provider: {str(e)}", code=500
        ).exception()
...
def create_or_update_payment_provider(request, provider_type="flutterwave"):
    ...
    try:
        ...
        if existing_providers:
            ...
            # Create new provider
            provider = registry.database(
                "PaymentProvider", "create", data=provider_data
            )
        ...
        return ResponseProvider(
            message=f"Error saving payment provider: {str(e)}", code=500
        ).exception()
