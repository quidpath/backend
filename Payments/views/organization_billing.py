...
def list_organization_invoices(request):
    ...
    try:
        ...
        invoices = OrganizationInvoice.objects.filter(**filter_kwargs).order_by('-created_at')[:50]
        ...
        return ResponseProvider(data={"invoices": invoices_data}, message="Invoices retrieved successfully", code=200).success()
        
    except Exception as e:
        logger.exception(f"Error listing organization invoices: {e}")
        return ResponseProvider(message=f"Error listing invoices: {str(e)}", code=500).exception()
...
def list_organization_payments(request):
    ...
    try:
        ...
        payments = OrganizationPayment.objects.filter(**filter_kwargs).order_by('-created_at')[:50]
        ...
        return ResponseProvider(data={"payments": payments_data}, message="Payments retrieved successfully", code=200).success()
        
    except Exception as e:
        logger.exception(f"Error listing organization payments: {e}")
        return ResponseProvider(message=f"Error listing payments: {str(e)}", code=500).exception()
...
def initiate_organization_payment(request):
    ...
    try:
        ...
        invoice = OrganizationInvoice.objects.filter(id=invoice_id, corporate_id=corporate_id).first()
        if not invoice:
            return ResponseProvider(message="Invoice not found", code=404).bad_request()
        
        if invoice.status == "paid":
            return ResponseProvider(message="Invoice already paid", code=400).bad_request()
        ...
        if payment_method == "mpesa":
            ...
            if not msisdn:
                return ResponseProvider(message="M-Pesa phone number is required", code=400).bad_request()
            ...
            if result.get("status") == "failed" or result.get("response_code") != "0":
                ...
                return ResponseProvider(
                    message=result.get("customer_message") or result.get("response_description") or "Failed to initiate payment",
                    code=400
                ).bad_request()
            
            payment.provider_reference = result.get("checkout_request_id") or result.get("provider_reference")
            ...
            return ResponseProvider(
                data={
                    ...
                    "status": "pending",
                    "message": result.get("customer_message", "Please check your phone for payment prompt"),
                },
            ...
            email = data.get("email") or corporate.get("email") or user.get("email", "billing@quidpath.com")
            ...
            payment.provider_reference = result.get("provider_reference") or result.get("tx_ref")
            payment.provider_metadata = result
            payment.save()
        ...
        return ResponseProvider(message=f"Error initiating payment: {str(e)}", code=500).exception()
...
def organization_payment_webhook(request):
    ...
    try:
        ...
        provider_reference = payload.get("data", {}).get("tx_ref") or payload.get("data", {}).get("flw_ref") or payload.get("tx_ref")
        ...
        # Find payment
        payment = OrganizationPayment.objects.filter(provider_reference=provider_reference).first()
        if not payment:
            logger.warning(f"Organization payment webhook: Payment not found for {provider_reference}")
        ...
        if not adapter.verify_webhook_signature(request.body, headers):
            logger.warning(f"Organization payment webhook: Invalid signature for {provider_reference}")
            return ResponseProvider(message="Invalid webhook signature", code=401).unauthorized()
        
        # Parse webhook
        webhook_data = adapter.handle_card_webhook(payload, headers) if payment.payment_method == "card" else adapter.handle_stk_callback(payload, headers)
        ...
        return ResponseProvider(message="Webhook processed successfully", code=200).success()
        
    except Exception as e:
        logger.exception(f"Error processing organization payment webhook: {e}")
        return ResponseProvider(message=f"Error processing webhook: {str(e)}", code=500).exception()
...
def get_subscription_status(request):
    ...
    try:
        ...
        if not subscription:
            ...
            if corporate:
                ...
                if created_at:
                    ...
                    created_date = parser.parse(created_at).date() if isinstance(created_at, str) else created_at
        ...
        return ResponseProvider(message=f"Error getting subscription status: {str(e)}", code=500).exception()
...
def get_invoice_details(request, invoice_id=None):
    ...
    try:
        ...
        payments = OrganizationPayment.objects.filter(invoice=invoice).order_by('-created_at')
        payments_data = []
        for payment in payments:
            payments_data.append({
                "id": str(payment.id),
        ...
        return ResponseProvider(message=f"Error getting invoice details: {str(e)}", code=500).exception()
