...
from Payments.models import PaymentProvider, RecordPayment
...
def initiate_card_payment(request):
    ...
    try:
        ...
        provider_config["callback_url"] = callback_url or provider_config.get(
            "callback_url", ""
        )
        ...
        if not provider_config.get("client_id"):
            return ResponseProvider(
                message="Flutterwave client_id not configured", code=400
            ).bad_request()
        if not provider_config.get("client_secret"):
            return ResponseProvider(
                message="Flutterwave client_secret not configured", code=400
            ).bad_request()
        if not provider_config.get("encryption_key"):
            return ResponseProvider(
                message="Flutterwave encryption_key not configured", code=400
            ).bad_request()
        ...
        metadata_dict = {
            ...
            "description": (
                f"Payment for invoice {invoice.get('number', '')}"
                if invoice
                else "Payment"
            ),
        ...
        if result["status"] == "failed":
            return ResponseProvider(
                message=result.get("message", "Failed to initiate payment"), code=400
            ).bad_request()
        ...
        with transaction.atomic():
            ...
            if not customer:
                return ResponseProvider(
                    message="Customer not found", code=400
                ).bad_request()
            ...
            if not accounts:
                return ResponseProvider(
                    message="Payment account not found", code=400
                ).bad_request()
            ...
            return ResponseProvider(
                data={
                    ...
                    "provider_reference": result.get("transaction_reference")
                    or result.get("transaction_id"),
                    "message": result.get("message")
                    or "Payment initiated successfully",
                },
                message="Payment initiated successfully",
                code=200,
            ).success()
        ...
        return ResponseProvider(
            message="An error occurred while initiating payment", code=500
        ).exception()
...
def card_webhook(request):
    ...
    try:
        ...
        provider_reference = payload.get("data", {}).get("tx_ref") or payload.get(
            "tx_ref"
        )
        ...
        if not payments:
            logger.warning(
                f"Flutterwave webhook: Payment not found for {provider_reference}"
            )
        ...
        if not payment_providers:
            logger.error(
                f"Flutterwave webhook: Provider not found for corporate {payment['corporate_id']}"
            )
            return ResponseProvider(
                message="Payment provider not found", code=400
            ).bad_request()
        ...
        try:
            if not adapter.verify_webhook_signature(request.body, headers):
                logger.warning(
                    f"Flutterwave webhook: Invalid signature for {provider_reference}"
                )
                return ResponseProvider(
                    ...
                    if invoices:
                        ...
                        registry.database(
                            ...
                            data={
                                ...
                                "payment_reference": webhook_data.get(
        ...
        return ResponseProvider(
            message="Webhook received successfully", code=200
        ).success()
