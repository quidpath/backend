...
class RecordPayment(BaseModel):
    ...
    account = models.ForeignKey(
        "Accounting.Account", on_delete=models.CASCADE
    )  # Deposit To
    reference_number = models.CharField(max_length=255, blank=True, null=True)
    provider_reference = models.CharField(
        max_length=255, blank=True, null=True
    )  # M-Pesa checkout_request_id, card transaction_id
    ...
    amount_excess = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal("0")
    )
...
class RecordPaymentLine(BaseModel):
    ...
    payment = models.ForeignKey(
        RecordPayment, on_delete=models.CASCADE, related_name="lines"
    )
...
class VendorPayment(BaseModel):
    ...
    created_by = models.ForeignKey(
        CorporateUser, on_delete=models.CASCADE, blank=True, null=True
    )
...
class VendorPaymentLine(BaseModel):
    ...
    payment = models.ForeignKey(
        VendorPayment, on_delete=models.CASCADE, related_name="lines"
    )
...
class PaymentProvider(BaseModel):
    ...
    corporate = models.ForeignKey(
        Corporate, on_delete=models.CASCADE, related_name="payment_providers"
    )
    ...
    config_json = models.JSONField(
        default=dict
    )  # Store API keys, secrets, etc. (encrypted in production)
    ...
    def __str__(self):
        return (
            f"{self.corporate.name} - {self.get_provider_type_display()} ({self.name})"
        )
