...
def login_user(request):
    ...
    if not username or not password:
        return _apply_api_security_headers(
            JsonResponse({"error": "Username and password required"}, status=400)
        )
    ...
    if not user.is_active and not user.is_superuser:
        return _apply_api_security_headers(
            JsonResponse({"error": "User is suspended"}, status=401)
        )

    if not check_password(password, user.password):
        return _apply_api_security_headers(
            JsonResponse({"error": "Invalid password"}, status=401)
        )
    ...
    if not user.is_superuser:
        otp_expired = not user.last_otp_sent_at or (
            timezone.now() - user.last_otp_sent_at > timedelta(hours=24)
        )
        if otp_expired:
            ...
            TransactionLogBase.log(
                "OTP_SENT", user=user, message="OTP sent due to expiration"
            )
...
def verify_otp(request):
    ...
    if not otp_code:
        return _apply_api_security_headers(
            JsonResponse({"error": "OTP required"}, status=400)
        )
    ...
    try:
        ...
        return _apply_api_security_headers(
            JsonResponse({"error": "Invalid OTP"}, status=400)
        )

    if not user.last_otp_sent_at or (
        now() - user.last_otp_sent_at > timedelta(hours=24)
    ):
        return _apply_api_security_headers(
            JsonResponse({"error": "OTP expired"}, status=400)
        )
    ...
    TransactionLogBase.log(
        "OTP_VERIFIED", user=user, message="OTP verified, login completed"
    )
...
def delete_user(request):
    ...
    TransactionLogBase.log(
        "USER_DELETED", user=None, message=f"User {username} deleted by admin/request"
    )
