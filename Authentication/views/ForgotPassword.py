...
def forgot_password(request):
    try:
        ...
        try:
            ...
            organisation_id = str(
                organisation.id if hasattr(organisation, 'id') else organisation) if organisation else None
        except CorporateUser.DoesNotExist:
            return ResponseProvider(message="User not found", code=404).bad_request()
        except Exception as e:
            return ResponseProvider(message=f"Error finding user: {str(e)}", code=500).exception()

        # Invalidate any existing valid OTP entries for this user using ServiceRegistry
        try:
            existing_otps = ServiceRegistry().database("forgotpassword", "filter", data={
                "user": user_id,
                "is_valid": True
            })
            ...
            for otp_item in existing_otps:
                ...
                otp_id = otp_entry.id if hasattr(otp_entry, 'id') else otp_entry.get("id") if hasattr(otp_entry,
                                                                                                      'get') else str(
                    otp_entry)
                ServiceRegistry().database("forgotpassword", "update",
                                           instance_id=otp_id,
                                           data={"is_valid": False})
        except Exception as e:
        ...
        try:
            forgot_password_entry = ServiceRegistry().database("forgotpassword", "create", data={
                "user": user_instance,
                "otp": otp,
                "is_valid": True
            })
        except Exception as e:
            return ResponseProvider(message=f"Error creating OTP entry: {str(e)}", code=500).exception()
        ...
        try:
            ...
            notification_handler.send_notification(notifications=[{
                "message_type": "2",
                "message": message,
                "destination": email,
            ...
            return ResponseProvider(message=f"Error sending notification: {str(e)}", code=500).exception()

        TransactionLogBase.log("OTP_SENT", user=user_instance, message="OTP sent successfully")

        return ResponseProvider(message="OTP sent to email", code=200).success()

    except Exception as e:
        return ResponseProvider(message=f"Error sending OTP: {str(e)}", code=500).exception()
...
def verify_pass_otp(request):
    try:
        ...
        otp_entries = ServiceRegistry().database("forgotpassword", "filter", data={
            "otp": otp,
            "is_valid": True,
            "is_verified": False
        })
        ...
        return ResponseProvider(message=f"Error verifying OTP: {str(e)}", code=500).exception()
...
def reset_password(request):
    ...
    try:
        ...
        if not new_password:
            return ResponseProvider(message="New password is required", code=400).bad_request()
        ...
        if not otp_entries:
            return ResponseProvider(message="No valid OTP session found", code=400).bad_request()
        ...
        if forgot_password_instance.has_expired():
            ...
            return ResponseProvider(message="OTP session has expired", code=400).bad_request()

        # Get user using email (if otp_entry["user"] is email)
        User = get_user_model()
        user_instance = User.objects.get(email=otp_entry["user"])  # Adjust if user ID is returned
        ...
        ServiceRegistry().database("forgotpassword", "delete",
            instance_id=otp_entry["id"]
        )
        ...
        return ResponseProvider(message=f"Error resetting password: {str(e)}", code=500).exception()
