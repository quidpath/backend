    ...
    try:
        ...
        if not user_id:
            return ResponseProvider(
                message="User ID is required", code=400
            ).bad_request()
        ...
        if not update_data:
            return ResponseProvider(
                message="No valid fields provided for corporate update", code=400
            ).bad_request()
        ...
        return ResponseProvider(
            message="Corporate profile updated successfully", code=200
        ).success()
        ...
        return ResponseProvider(
            message=f"Error updating corporate profile: {str(e)}", code=500
        ).exception()
