...
def generate_pl_data(registry, corporate_id, start_date, end_date):
    ...
    if revenue_types:
        ...
        subtype_dict = {}
        if account_ids:
            accounts = registry.database("Account", "filter", data={"id__in": account_ids, "corporate_id": corporate_id})
            subtype_dict = {acc["id"]: acc.get("account_sub_type__name", "Other Revenue") for acc in accounts}
        revenue_dict = defaultdict(Decimal)
        for line in lines:
            sub_name = subtype_dict.get(line["account_id"], "Other Revenue")
            balance = Decimal(str(line.get("credit", 0))) - Decimal(str(line.get("debit", 0)))
    ...
    if expense_types:
        ...
        if account_ids:
            accounts = registry.database("Account", "filter", data={"id__in": account_ids, "corporate_id": corporate_id})
            subtype_dict = {acc["id"]: acc.get("account_sub_type__name", "Other Expense") for acc in accounts}
        expense_dict = defaultdict(Decimal)
        for line in lines:
            sub_name = subtype_dict.get(line["account_id"], "Other Expense")
            balance = Decimal(str(line.get("debit", 0))) - Decimal(str(line.get("credit", 0)))
...
def get_balance_sheet_data(registry, corporate_id, end_date, start_date=None):
    ...
    if asset_types:
        ...
        if account_ids:
            accounts = registry.database("Account", "filter", data={"id__in": account_ids, "corporate_id": corporate_id})
            subtype_dict = {acc["id"]: acc.get("account_sub_type__name") for acc in accounts if acc.get("account_sub_type__name")}
    ...
    if equity_types:
        ...
        if start_date:
            retained_key = next((k for k in equity_dict if 'retained' in k.lower() and 'earnings' in k.lower()), None)
            if retained_key:
    ...
    data = {
        'assets': {
            'subtypes': assets_subtypes,
            'total': float(total_assets)
        },
        'liabilities': {
            'subtypes': liab_subtypes,
...
def generate_cash_flow_data(registry, corporate_id, start_date, end_date):
    ...
    delta_assets_ex_cash = (total_assets_end - cash_end) - (total_assets_start - cash_start)
...
def generate_profit_loss_report(request):
    ...
    try:
        ...
        if not start_date_str or not end_date_str:
            return ResponseProvider(message="Start date and end date are required", code=400).bad_request()
        ...
        return ResponseProvider(message="An error occurred while generating report", code=500).exception()
...
def generate_balance_sheet_report(request):
    ...
    try:
        ...
        start_date_str = data.get("start_date")
        if not end_date_str:
            return ResponseProvider(message="End date is required", code=400).bad_request()

        start_date = date.fromisoformat(start_date_str) if start_date_str else None
        end_date = date.fromisoformat(end_date_str)

        report_data = get_balance_sheet_data(registry, corporate_id, end_date, start_date)
...
def generate_cash_flow_report(request):
    ...
    try:
        ...
        report_data = generate_cash_flow_data(registry, corporate_id, start_date, end_date)
...
def retrieve_financial_report(request):
    ...
    try:
        ...
        if not report_id:
            return ResponseProvider(message="Report ID is required", code=400).bad_request()
        ...
        if not reports:
            return ResponseProvider(message="Report not found for this corporate", code=404).bad_request()
        ...
        return ResponseProvider(message="An error occurred while retrieving report", code=500).exception()
...
def download_financial_report(request):
    ...
    try:
        ...
        if report_type in ["PROFIT_LOSS", "INCOME_STATEMENT"]:
            ...
            writer.writerow(['', 'Total Revenues', report_data.get('total_revenues', 0)])
            ...
            for desc, amount in report_data.get('assets', {}).get('subtypes', {}).items():
                writer.writerow(['', desc, amount])
            writer.writerow(['Total Assets', '', report_data.get('assets', {}).get('total', 0)])

            writer.writerow(['Liabilities', '', ''])
            for desc, amount in report_data.get('liabilities', {}).get('subtypes', {}).items():
                writer.writerow(['', desc, amount])
            writer.writerow(['Total Liabilities', '', report_data.get('liabilities', {}).get('total', 0)])

            writer.writerow(['Equity', '', ''])
            for desc, amount in report_data.get('equity', {}).get('subtypes', {}).items():
                writer.writerow(['', desc, amount])
            writer.writerow(['Total Equity', '', report_data.get('equity', {}).get('total', 0)])

        elif report_type == "CASH_FLOW":
            writer.writerow(['Cash Flow Statement', '', ''])
            writer.writerow(['Operating Activities', report_data.get('operating_cash_flow', 0)])
            writer.writerow(['Net Income', report_data.get('net_income', 0)])

            writer.writerow(['Investing Activities', report_data.get('investing_cash_flow', 0)])

            writer.writerow(['Financing Activities', report_data.get('financing_cash_flow', 0)])

            writer.writerow(['Net Change in Cash', report_data.get('net_change_in_cash', 0)])
        ...
        return ResponseProvider(message="An error occurred while downloading report", code=500).exception()