...
def list_recurring_transactions(request):
    ...
    try:
        ...
        transactions = registry.database(
            model_name="RecurringTransaction", operation="filter", data=filter_data
        )
        ...
        return ResponseProvider(
            message=f"Error listing recurring transactions: {str(e)}", code=500
        ).exception()
...
def update_recurring_transaction(request, transaction_id):
    ...
    try:
        ...
        transaction = registry.database(
            "RecurringTransaction", "get", data={"id": transaction_id}
        )
        if not transaction:
            return ResponseProvider(
                message="Recurring transaction not found", code=404
            ).bad_request()
        ...
        return ResponseProvider(
            message=f"Error updating recurring transaction: {str(e)}", code=500
        ).exception()
