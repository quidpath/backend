"""
Invoice draft/post state machine views.
"""
...
from Accounting.models.sales import Invoices, InvoiceLine
...
def post_invoice(request, invoice_id):
    ...
    try:
        qs = Invoices.objects.filter(id=invoice_id, corporate_id=corp_id)
        if not qs.exists():
        ...
        errors = []
        if not inv.customer_id: errors.append("Customer required.")
        if not InvoiceLine.objects.filter(invoice=inv).exists(): errors.append("No line items.")
        if errors:
            return ResponseProvider(message="; ".join(errors), code=400).bad_request()
        with transaction.atomic():
            inv.status = "POSTED"
            inv.posted_at = timezone.now()
            inv.posted_by_id = corp_user_id
            inv.save()
        result = {"id": str(inv.id), "number": inv.number, "status": inv.status,
                  "posted_at": inv.posted_at.isoformat()}
        return ResponseProvider(message="Invoice posted", data=result, code=200).success()
...
@csrf_exempt
@require_module_permission("finance")
def auto_save_invoice(request, invoice_id):
    data, metadata = get_clean_data(request)
    registry = ServiceRegistry()
    ...
    try:
        ...
        inv = qs.first()
        if inv.status == "POSTED":
            return ResponseProvider(message="Cannot edit posted invoice", code=403).bad_request()
        changed = []
...
@csrf_exempt
@require_module_permission("finance")
def list_draft_invoices(request):
    data, metadata = get_clean_data(request)
    registry = ServiceRegistry()
    uid, corp_id, _ = _corp_user(metadata, registry)
    if not uid:
        return ResponseProvider(message="User not authenticated", code=401).unauthorized()
    try:
        drafts = list(Invoices.objects.filter(corporate_id=corp_id, status="DRAFT")
                      .values("id","number","date","due_date","status","drafted_at","customer_id"))
        return ResponseProvider(message="OK", data={"invoices": drafts, "total": len(drafts)}, code=200).success()
    except Exception as e:
        return ResponseProvider(message=f"Error: {e}", code=500).exception()
