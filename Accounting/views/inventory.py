...
def create_warehouse(request):
    ...
    try:
        ...
        if not warehouse_data["name"]:
            return ResponseProvider(
                message="Warehouse name is required", code=400
            ).bad_request()
        ...
        return ResponseProvider(
            data=warehouse, message="Warehouse created successfully", code=200
        ).success()

    except Exception as e:
        logger.exception(f"Error creating warehouse: {e}")
        return ResponseProvider(
            message=f"Error creating warehouse: {str(e)}", code=500
        ).exception()
...
def list_warehouses(request):
    ...
    try:
        ...
        return ResponseProvider(
            message=f"Error listing warehouses: {str(e)}", code=500
        ).exception()
...
def update_warehouse(request, warehouse_id):
    ...
    try:
        ...
        if not warehouse:
            return ResponseProvider(
                message="Warehouse not found", code=404
            ).bad_request()
        ...
        updated_warehouse = registry.database(
            "Warehouse", "update", instance_id=warehouse_id, data=update_data
        )
        ...
        return ResponseProvider(
            data=updated_warehouse, message="Warehouse updated successfully", code=200
        ).success()

    except Exception as e:
        logger.exception(f"Error updating warehouse: {e}")
        return ResponseProvider(
            message=f"Error updating warehouse: {str(e)}", code=500
        ).exception()
...
def delete_warehouse(request, warehouse_id):
    ...
    try:
        ...
        # Soft delete
        registry.database(
            "Warehouse", "update", instance_id=warehouse_id, data={"is_active": False}
        )
        ...
        return ResponseProvider(
            message="Warehouse deleted successfully", code=200
        ).success()

    except Exception as e:
        logger.exception(f"Error deleting warehouse: {e}")
        return ResponseProvider(
            message=f"Error deleting warehouse: {str(e)}", code=500
        ).exception()
...
def create_inventory_item(request):
    ...
    try:
        ...
        if not item_data["name"] or not item_data["sku"]:
            return ResponseProvider(
                message="Item name and SKU are required", code=400
            ).bad_request()

        # Check if SKU already exists
        existing = registry.database(
            "InventoryItem", "filter", data={"sku": item_data["sku"]}
        )
        if existing:
            return ResponseProvider(
                message="SKU already exists", code=400
            ).bad_request()
        ...
        return ResponseProvider(
            data=item, message="Inventory item created successfully", code=200
        ).success()

    except Exception as e:
        logger.exception(f"Error creating inventory item: {e}")
        return ResponseProvider(
            message=f"Error creating inventory item: {str(e)}", code=500
        ).exception()
...
def list_inventory_items(request):
    ...
    try:
        ...
        return ResponseProvider(
            message=f"Error listing inventory items: {str(e)}", code=500
        ).exception()
...
def update_inventory_item(request, item_id):
    ...
    try:
        ...
        if not item:
            return ResponseProvider(
                message="Inventory item not found", code=404
            ).bad_request()
        ...
        if "sku" in data and data["sku"] != item.get("sku"):
            existing = registry.database(
                "InventoryItem", "filter", data={"sku": data["sku"]}
            )
            if existing:
                return ResponseProvider(
                    message="SKU already exists", code=400
        ...
        updated_item = registry.database(
            "InventoryItem", "update", instance_id=item_id, data=update_data
        )
        ...
        return ResponseProvider(
            data=updated_item, message="Inventory item updated successfully", code=200
        ).success()

    except Exception as e:
        logger.exception(f"Error updating inventory item: {e}")
        return ResponseProvider(
            message=f"Error updating inventory item: {str(e)}", code=500
        ).exception()
...
def delete_inventory_item(request, item_id):
    ...
    try:
        ...
        # Soft delete
        registry.database(
            "InventoryItem", "update", instance_id=item_id, data={"is_active": False}
        )
        ...
        return ResponseProvider(
            message="Inventory item deleted successfully", code=200
        ).success()

    except Exception as e:
        logger.exception(f"Error deleting inventory item: {e}")
        return ResponseProvider(
            message=f"Error deleting inventory item: {str(e)}", code=500
        ).exception()
...
def create_stock_movement(request):
    ...
    try:
        ...
        movement_data = {
            ...
            "unit_cost": Decimal(str(data.get("unit_cost", 0))),
            "total_cost": Decimal(str(data.get("quantity", 0)))
            * Decimal(str(data.get("unit_cost", 0))),
        ...
        if not all(
            ...
            return ResponseProvider(
                message="Warehouse, item, and movement type are required", code=400
            ).bad_request()
        ...
        with transaction.atomic():
            ...
            if movement_data["status"] == "posted":
                item = registry.database(
                    "InventoryItem", "get", data={"id": movement_data["item_id"]}
                )
                if item:
                    ...
                    if item.get("valuation_method") == "average_cost":
                        ...
                        registry.database(
                            "InventoryItem",
                            "update",
                            instance_id=movement_data["item_id"],
                            data={"quantity_on_hand": new_qty, "average_cost": new_avg},
                        )
                    else:
                        registry.database(
                            "InventoryItem",
        ...
        return ResponseProvider(
            data=movement, message="Stock movement created successfully", code=200
        ).success()

    except Exception as e:
        logger.exception(f"Error creating stock movement: {e}")
        return ResponseProvider(
            message=f"Error creating stock movement: {str(e)}", code=500
        ).exception()
...
def list_stock_movements(request):
    ...
    try:
        ...
        return ResponseProvider(
            message=f"Error listing stock movements: {str(e)}", code=500
        ).exception()
