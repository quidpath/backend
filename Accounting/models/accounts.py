...
class AccountType(BaseModel):
    ...
    normal_balance = models.CharField(max_length=6, choices=[('DEBIT', 'Debit'), ('CREDIT', 'Credit')], blank=True, null=True)
...
class Account(BaseModel):
    ...
    """
    corporate = models.ForeignKey(Corporate, on_delete=models.CASCADE, related_name='accounts')
    ...
    parent_account = models.ForeignKey('self', on_delete=models.CASCADE, blank=True, null=True,
    ...
    def clean(self):
        if self.account_sub_type and self.account_sub_type.account_type != self.account_type:
            raise ValidationError("Account sub-type must belong to the selected account type")
...
class JournalEntry(BaseModel):
    ...
    """
    corporate = models.ForeignKey(Corporate, on_delete=models.CASCADE, related_name='journal_entries')
    date = models.DateField()
    reference = models.CharField(max_length=50)
    description = models.TextField(blank=True, default='')
    source_type = models.CharField(max_length=50, blank=True)  # 'invoice', 'vendor_bill', 'manual', etc.
...
class JournalEntryLine(BaseModel):
    ...
    def clean(self):
        ...
        if self.account.corporate != self.journal_entry.corporate:
            raise ValidationError("Account must belong to the same corporate as the journal entry")
...
class Expense(BaseModel):
    ...
    corporate = models.ForeignKey(Corporate, on_delete=models.CASCADE, related_name='expenses')
    date = models.DateField()
    reference = models.CharField(max_length=50)
    ...
    payment_account = models.ForeignKey(Account, on_delete=models.PROTECT, related_name='expense_payments')

    # Optional vendor link
    vendor = models.ForeignKey("Accounting.Vendor", on_delete=models.SET_NULL, blank=True, null=True, related_name='expenses')

    # Tax information
    tax_amount = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'))
    tax_rate = models.ForeignKey("Accounting.TaxRate", on_delete=models.PROTECT, blank=True, null=True)

    # Journal entry
    journal_entry = models.ForeignKey(JournalEntry, on_delete=models.SET_NULL, blank=True, null=True)
